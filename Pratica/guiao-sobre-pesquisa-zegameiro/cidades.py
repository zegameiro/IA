...
class Cidades(SearchDomain):
    ...
    def cost(self, city, action):
        ...
        for (C1,C2,cost) in self.connections:
            ...
            if (C1,C2) == action or (C1 == action[1] and C2 == action[0]):
                total_cost = cost
                break
            else:
                total_cost = None
    ...
    def heuristic(self, city, goal_city):
        coordInitial = self.coordinates[city]
        coordGoal = self.coordinates[goal_city]
        return math.dist(coordInitial, coordGoal)
...
print(timeit.timeit(lambda: t.search()))
